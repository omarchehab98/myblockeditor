 ...
 * @link: https://github.com/colorhook/jsxml
 * @version:0.7.0
 */
 ...
 * HTML Parser
 * @author: John Resig
 * @link: http://ejohn.org/files/htmlparser.js
 */

/**
 * Original HTML Parser
 * @author: Erik Arvidsson
 * @link: http://erik.eae.net/simplehtmlparser/simplehtmlparser.js
 */
...
merge(XML.prototype, {
  ...
  addAttribute: function(attribute) {
    var attr = new XML();
    attr._parent = this;
    attr._nodeKind = 'attribute';
    var _qname;
    if (attribute.name === 'xmlns') {
      _qname = new QName(new Namespace('xmlns', attribute.value), '');
    } else {
      _qname = QName._format(attribute.name, attribute.value);
    }
    var prefix = _qname._ns.prefix || "";
    ...
    if (prefix === 'xmlns') {
      var ns = new Namespace(_qname.localName, _qname.uri);
      this.addNamespace(ns);
      if (_qname.localName === this._qname._ns.prefix) {
        this.setNamespace(ns);
      }
    } else {
      attr._qname = _qname;
      attr._text = attribute.value;
  ...
  removeAttribute: function(name) {
    try {
      var that = this;
      this.attributes().each(function(attr, i) {
        if (attr.localName() === name) {
          that._attributes.splice(i, 1);
...
export {containsEntity, replaceToEntity, replaceFromEntity, _parseXML, Namespace, QName, NodeKind, XMLList, XML};
